...
const storeTokenRecette = 'g58kzdxrvdtlhnumiv3wd93z95hq6khm';
...
const buildReleases = () => {
  const cmd = 'cordova build android ios --device';
...
const deployReleases = () => {
  const cmds = [
    ...
    `${paths.smartphoneDir}/platforms/android/build/outputs/apk/android-debug.apk`,
    `${paths.tabletDir}/platforms/ios/build/device/${tabletAppName}.ipa`,
    `${paths.tabletDir}/platforms/android/build/outputs/apk/android-debug.apk`
  ].map(filepath => `ruby ${paths.rootDir}/appaloosa-client.rb ${storeTokenRecette} ${filepath}`);

  return cmds.reduce((executePromise, cmd) => {
